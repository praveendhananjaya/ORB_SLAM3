...
#include "prof.h"
#include "profTime.h"
#define myoptimize 'Goptimize'
#define setKey "set_key"
#define setMap "set_Map"
...
#define opt "Optimize1"
    // Optimize!
    {

    profTime profTime(opt);
    optimizer.setVerbose(false);
    optimizer.initializeOptimization();

    //profTime profTime(reinterpret_cast<const char *>(myoptimize));
...
#define POSE_OPTIMIZATION 1
#if POSE_OPTIMIZATION
#include <fstream>

    ofstream file1;
    file1.open("input.txt", ios::app);
...
#define LocalMap "LocalMap"
    {
        profTime profTime(LocalMap);
...
#define SetLocalKeyFrame "SetLocalKeyFrame"
    {
        profTime profTime(SetLocalKeyFrame);
...
#define SetFixedKeyFrame "SetFixedKeyFrame"
    {
        profTime profTime(SetFixedKeyFrame);
...
#define SetMapPointvertices "SetMapPointvertices"
...
#define MapPointVec "MapPointVec"
    {
        profTime profTime(MapPointVec);
        vpEdgesMono.reserve(nExpectedSize);
        vpEdgesBody.reserve(nExpectedSize);
        vpEdgeKFMono.reserve(nExpectedSize);
        vpEdgeKFBody.reserve(nExpectedSize);
...
#define GOptimizer "GOptimizer"
    {
        profTime profTime(GOptimizer);
...
#define CheckInlierObservations  "CheckInlierObservations"
    {
        profTime profTime(CheckInlierObservations);
        for (size_t i = 0, iend = vpEdgesMono.size(); i < iend; i++) {
...
#define MapMutex "MapMutex"
    ...
    // Recover optimized data
    //Keyframes
#define RecoverOptimized "RecoverOptimized"
    {
        profTime profTime(Fixed);
        for (list<KeyFrame *>::iterator lit = lLocalKeyFrames.begin(), lend = lLocalKeyFrames.end();
             lit != lend; lit++) {
